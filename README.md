...
| [`ebt/ebt-v7.1.compact`](./ebt/ebt-v7.1.compact) | EBT v7.1 — v7 + `DividendMintedEntry` public ledger log on dividend-slice mint, plus multisig-gated `setLivingDividendAddress` and `clearLivingDividendAddress` setters. Nullable-default LD pointer means v7.1 is functionally identical to v7 until multisig binds it. Enables the Living Dividend downstream contract via a keeper subscribing to the log. See [`ebt/V7.1-EVENT-DIFF.md`](./ebt/V7.1-EVENT-DIFF.md). | 🛠️ **STAGED** — syntax-clean on compactc 0.31.0. Awaits deploy ceremony as part of the Living Dividend rollout. |
